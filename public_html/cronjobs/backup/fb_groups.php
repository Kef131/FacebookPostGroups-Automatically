		...
		//end if video
			...
			///POST NOW
				...
				if($postallgroups==true) //loop groups
				{
					...
					//logs
					sleep(1);
					mysql_query("INSERT INTO fbshare_logs VALUES ('','".$fbaccountid."','".$campaignid."','<font color=#FF3700>Posting message[batch message]</font><br>','".date('Y-m-d H:i:s')."')");
					sleep(1);
					...
					while($postinallgroupsdetails=mysql_fetch_array($allgroups))
					{
							...
							///batch body for regular message
									...
									if(!isset($_SESSION['lastphotoid'])) //session not set -> upload image
									{
									 ...
									 echo "Upload image to group: [".$resgroupdetailsres['fbpageurl']."]<br>";
										...
										{
										echo "<br>-----------------------<br>Error while uploading first image<br>-----------------------<br>";
									...
									else
									{
										...
										if($messagespostedthistome%3==0) //link
										{	
							...
							VALUES ('','".$fbaccountid."','".$campaignid."','Put group ID[".$resgroupdetailsres['fbpageurl']."] in queue...Message to post in this group:<br>".mysql_real_escape_string($status_logs)."','".date('Y-m-d H:i:s')."')");
									...
									if((((($totalgroupstopostin-$k)%20)==0) && $k>1) || ($flush_array==true))
									{
										...
										unset($batcharray);
										$batcharray=array();								
										mysql_query("INSERT INTO fbshare_logs VALUES ('','".$fbaccountid."','".$campaignid."','<font color=#FF3700>Successfully posted in groups IDs: <br>".$logs_batch_groups."</font>','".date('Y-m-d H:i:s')."')");
										$logs_batch_groups="";
										}
							...
							else //IF REGULAR POST
							{
									...
									if($isrss==false)
									{
										if($messagespostedthistome%3==0 || $messagespostedthistome<10) //if odd  OR max mes posted < 10
										{
											$body = array
											(
									...
									if($isrss==true)
									{
										...
										if($messagespostedthistome%3==0 || $messagespostedthistome<10) //if even OR max mes posted < 10
										{
											$body = array
											(
											'message'        => $status_rss,
