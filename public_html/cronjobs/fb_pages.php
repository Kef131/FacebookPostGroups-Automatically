...
exit();
	...
	if($fbaccountok>0) //login accepted
	{
		...
		{
			echo "<br>Invalid page ID. Please check again your page ID. Campaign paused.<br>-----------------<br>"; 
			mysql_query("INSERT INTO fbshare_logs VALUES ('','".$fbaccountid."','".$campaignid."','Invalid page ID. Please check again your page ID. Campaign paused.','".date('Y-m-d H:i:s')."')");
			mysql_query("UPDATE fbshare_campaigns SET campaign_enabled='0' WHERE campaignid='".$campaignid."'");
			$nomessages=0;
			return $nomessages;
			exit();
		...
		//token empty
		if(trim($page_at)=="") ///Empty token
		{
			echo "<br>Invalid page ID or no rights to post on this page. Campaign paused.<br>-----------------<br>"; 
			mysql_query("INSERT INTO fbshare_logs VALUES ('','".$fbaccountid."','".$campaignid."','Invalid page ID or no rights to post on this page. Campaign paused.','".date('Y-m-d H:i:s')."')");
			mysql_query("UPDATE fbshare_campaigns SET campaign_enabled='0' WHERE campaignid='".$campaignid."'");
			$nomessages=0;
			return $nomessages;
			exit();
		...
		///POST NOW
			...
			$post = $fb->postOnPage($pageid,$page_at,$new_status,$url_to_send);
				...
				{
				echo "<br>Try to post message[page update]: ".$status_logs."<br>";
				echo "--------------------------------------<br>";
...
$rescampaigns=mysql_query("SELECT * FROM fbshare_campaigns WHERE isgroup=0 AND campaign_enabled=1 AND is_campaign_finished=0 AND messagespostedon!=0 ");
...
$rescampaigns=mysql_query("SELECT * FROM fbshare_campaigns WHERE isgroup=0 AND campaign_enabled=1 AND is_campaign_finished=0 AND messagespostedon!=0 AND campaign_run=1 AND ((TIMESTAMPDIFF(MINUTE, campaign_last_time_run, now()) > 60) || campaign_last_time_run='0000-00-00 00:00:00') ORDER BY campaign_last_time_run ASC");
	$total_pending_campaigns=mysql_num_rows($rescampaigns);
...
$rescampaigns=mysql_query("SELECT * FROM fbshare_campaigns WHERE isgroup=0 AND campaign_enabled=1 AND is_campaign_finished=0 AND messagespostedon!=0 AND campaign_run=7 AND ((TIMESTAMPDIFF(HOUR, campaign_last_time_run, now()) > 24) || campaign_last_time_run='0000-00-00 00:00:00') ORDER BY campaign_last_time_run ASC");
	$total_pending_campaigns=mysql_num_rows($rescampaigns);
...
$rescampaigns=mysql_query("SELECT * FROM fbshare_campaigns WHERE isgroup=0 AND campaign_enabled=1 AND is_campaign_finished=0 AND messagespostedon!=0 AND campaign_run=8 AND campaign_run_day_post_x_messages='".$todaydate_daynr."' AND DATE(campaign_last_time_run)!='".$todaydate."' ORDER BY campaign_last_time_run ASC");
...
//priority 5: 1 message in the list, on exact date
$todaydate=date("Y-m-d");
$rescampaigns=mysql_query("SELECT * FROM fbshare_campaigns WHERE isgroup=0 AND campaign_enabled=1 AND is_campaign_finished=0 AND messagespostedon!=0 AND campaign_run=3  AND campaign_run_specific_day='".$todaydate."' AND DATE(campaign_last_time_run)!='".$todaydate."' ORDER BY campaign_last_time_run ASC");
...
//priority 6: 1 in the list, on exact date(X day)
$todaydate=date("Y-m-d");
$todaydate_monthnr=date("j");

$rescampaigns=mysql_query("SELECT * FROM fbshare_campaigns WHERE isgroup=0 AND campaign_enabled=1 AND is_campaign_finished=0 AND messagespostedon!=0 AND campaign_run=5 AND campaign_run_month_day='".$todaydate_monthnr."' AND DATE(campaign_last_time_run)!='".$todaydate."' ORDER BY campaign_last_time_run ASC");
...
$rescampaigns=mysql_query("SELECT * FROM fbshare_campaigns WHERE isgroup=0 AND campaign_enabled=1 AND is_campaign_finished=0 AND messagespostedon!=0 AND campaign_run=0 ORDER BY campaign_last_time_run ASC");
