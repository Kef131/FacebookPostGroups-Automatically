...
if($total_pages>0)
{
	...
	{
	$checked=" ";
	if($_GET['pageid']==$pagesdetails['pageid'])// on SPECIFIC PAGE
	{
		$checked=" checked";
	}
	
$text.="<input onclick='document.forms[0].isgroup.value=0;' type=radio name=messagespostedon value=".$pagesdetails['pageid']." ".$checked."> <font style='color:blue; font-weight:bold'>En la fan page: </font>".$pagesdetails['fbpagedescription']."<br>";
	}
...
if($total_groups>0)
{
	...
	if($_GET['pageid']=='-1')// on GROUP SELECTED
	{
		$checked=" checked";
	}
	
	$text.="<input onclick='document.forms[0].isgroup.value=1;' type=radio name=messagespostedon value=-1 ".$checked."> <font style='color:red; font-weight:bold'>En los Grupos (Selecciona al menos un grupo):</font><br>";
	...
	{
		$group_checked="";
		$res_is_group_check=mysql_query("SELECT * FROM fbshare_group_campaigns WHERE groupid='".$pagesdetails['pageid']."' AND campaignid='".$_GET['campaignid']."' ");
		$isgrochecked=mysql_num_rows($res_is_group_check);
		if($isgrochecked==1) {$group_checked=" checked";} //YES
	
	$text.="<input type=checkbox name=groupid_".$gr_indx." value=".$pagesdetails['pageid']." ".$group_checked."> ".$pagesdetails['fbpagedescription']."<br>";
	$gr_indx++;
	}
