...
if($total_groups>0)
{
	$text.="<input onclick='document.forms[0].isgroup.value=1;' type=radio name=messagespostedon value=-1 > <font style='color:red; font-weight:bold'>En Grupos de Facebook (SELECCIONA UN M&Aacute;XIMO DE 10 GRUPOS):</font><br>";
	
	$gr_indx=0;
	$text.="<div style=\"width: 660px;margin:0px;margin-left:24px; padding:0px;\"><input type=checkbox name=checkall onClick=togglechecked()> <font style='font-weight:bold'>MARCAR / DESMARCAR TODOS LOS GRUPOS</font></div>";
	
	///scroll
	if($total_groups>15)
	{
	$text.="<div id=\"scrollgroups\" style=\"overflow: auto; width: 660px; max-height: 285px; margin:0px;margin-left:24px; padding:0px; border-top:1px #3a5896 solid; overflow-y:scroll;overflow-x:hidden;\">";
	}
	else
	{
	$text.="<div style=\"width: 660px; max-height: 285px; margin:0px;margin-left:24px; padding:0px; border-top:1px #3a5896 solid;\">";
	}
	//end scroll
	
	while($pagesdetails=mysql_fetch_array($result_gr))
	{
	$text.="<input type=checkbox name=groupid_".$gr_indx." value=".$pagesdetails['pageid']."> ".$pagesdetails['fbpagedescription']."<br>";
	$gr_indx++;
	}
